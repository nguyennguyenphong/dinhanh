...
class APIToken(BaseModel):
    ...
    branch_id = models.IntegerField(
        null=True, blank=True, help_text="Optional branch ID for branch-specific tokens"
    )
    ...
    request_count = models.BigIntegerField(
        default=0, help_text="Total number of requests made with this token"
    )
    ...
    is_active = models.BooleanField(
        default=True, db_index=True, help_text="Token is active and can be used"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["is_active"], name="idx_api_tokens_active"),
            # Index for token prefix lookup
            models.Index(fields=["token_prefix"], name="idx_api_tokens_prefix"),
            # Index for tenant and active
            models.Index(
                fields=["tenant", "is_active"], name="idx_api_tokens_tenant_active"
            ),
            # Index for expiration queries
            models.Index(fields=["expires_at"], name="idx_api_tokens_expires"),
        ]
    ...
    def get_active_tokens(cls, tenant):
        ...
        """
        return cls.objects.filter(tenant=tenant, is_active=True).filter(
    ...
    def get_expired_tokens(cls, tenant):
        ...
        Returns:
            QuerySet of APIToken objects
        """
        return cls.objects.filter(tenant=tenant, expires_at__lt=timezone.now())
    ...
    def cleanup_expired_tokens(cls, days=90):
        ...
        expired_tokens = cls.objects.filter(expires_at__lt=cutoff_date, is_active=False)
