...
class SystemConfig(BaseModel):
    ...
    label = models.CharField(
        max_length=255, help_text="Human-readable label for the configuration"
    )
    description = models.TextField(
        blank=True, null=True, help_text="Detailed description of what this config does"
    )
    ...
    is_encrypted = models.BooleanField(
        default=False, help_text="Value is encrypted (for sensitive data)"
    )
    is_public = models.BooleanField(
        default=False, help_text="Configuration is exposed to frontend"
    )
    is_readonly = models.BooleanField(
        default=False, help_text="Configuration is read-only (locked in production)"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["env"], name="idx_config_env"),
        ]
    ...
    def get_config(cls, tenant, category, key, use_cache=True):
        try:
            return cls.objects.get(tenant=tenant, category=category, key=key)
