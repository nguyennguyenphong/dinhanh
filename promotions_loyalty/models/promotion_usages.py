...
class PromotionUsage(models.Model):
    ...
    def __str__(self):
        ...
        customer_label = f"Cust #{self.customer_id}" if self.customer_id else "ANONYMOUS"
    ...
    def clean(self):
        ...
        if self.booking_id and self.discount_applied:
            ...
            if hasattr(self.booking, 'total_amount') and self.discount_applied > self.booking.total_amount:
                raise ValidationError({
                    'discount_applied': _("Financial Anomaly Error: Applied discount magnitude cannot exceed the gross order ticket value.")
                })
    ...
    def save(self, *args, **kwargs):
        ...
        if is_creating:
            ...
            if promotion_instance.usage_limit and promotion_instance.usage_count >= promotion_instance.usage_limit:
                raise ValidationError(_("Campaign Outage Exception: The target coupon limit has been fully claimed just before lock execution."))
