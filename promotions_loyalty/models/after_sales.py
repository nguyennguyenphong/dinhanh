...
class AfterSales(BaseModel):
    ...
    TYPE_CHOICES = (
        (
            "VOUCHER",
            _("Voucher - High-fidelity electronic balance debit card generation"),
        ),
        (
            "LOYALTY_POINTS",
            _(
        ...
        (
            "GIFT",
            _("Gift - Physical merchandise line item inventory claiming authorization"),
        ),
        (
            "DISCOUNT_CODE",
            _(
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=models.Q(
                    type__in=["VOUCHER", "LOYALTY_POINTS", "GIFT", "DISCOUNT_CODE"]
                ),
    ...
    def clean(self):
        ...
        if not isinstance(self.conditions, dict):
            raise ValidationError(
                {
                    "conditions": _(
                        "Data Structure Error: The parameters structural rule mapping field must evaluate as a valid JSON object."
                    )
                }
            )
        ...
        if self.type in ["VOUCHER", "LOYALTY_POINTS"] and self.value is None:
            raise ValidationError(
                {
                    "value": _(
                        "Compliance Exception: Financial rewards like points or monetary vouchers must possess an explicit value parameter."
                    )
                }
            )
    ...
    def evaluate_customer_qualification(self, customer_historical_context_dict):
        ...
        for key, required_rule_value in self.conditions.items():
            if key in customer_historical_context_dict:
                ...
                if (
                    key.startswith("min_")
                    and customer_historical_context_dict[key] < required_rule_value
                ):
                    return False
