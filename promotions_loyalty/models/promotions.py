...
class Promotion(models.Model):
    ...
    DISCOUNT_TYPE_CHOICES = (
        (
            "PERCENT",
            _("Percentage - Scaled mathematical deduction bounded by max limits"),
        ),
        (
            "FIXED_AMOUNT",
            _("Fixed Amount - Absolute invariant commercial currency value deduction"),
        ),
        (
            "FREE_SEAT",
            _("Free Seat - Total asset valuation waiver for applied line units"),
        ),
    )
        ...
        if self.valid_from and self.valid_to:
            if self.valid_from >= self.valid_to:
                raise ValidationError(
                    {
                        "valid_to": _(
                            "Campaign Error: Target expiration timestamp node must physically succeed activation valid_from landmarks."
                        )
                    }
                )
        ...
        if self.discount_type == "PERCENT":
            if self.discount_value > 100.00:
                raise ValidationError(
                    {
                        "discount_value": _(
                            "Financial Limit Error: Percentage-based calculations cannot mathematically scale above a 100.00% ceiling parameter."
                        )
                    }
                )
    ...
    def is_currently_valid(self, order_amount=0.00, sales_channel=None):
        ...
        if not self.is_active:
            return False, _(
                "This promotional coupon has been administratively disabled."
            )

        # Gate B: Chronology boundary checks
        if now < self.valid_from:
            return False, _(
                "This promotional coupon campaign has not reached its official activation date yet."
            )
        if self.valid_to and now > self.valid_to:
            return False, _(
                "This promotional coupon code has reached its formal expiration date."
            )

        # Gate C: Exhaustion limits check
        if self.usage_limit and self.usage_count >= self.usage_limit:
            return False, _(
                "This coupon allocation budget has been completely exhausted system-wide."
            )

        # Gate D: Cart baseline qualification check
        if self.min_order_amount and Decimal(str(order_amount)) < self.min_order_amount:
            return False, _(
                f"Cart value is below qualification benchmarks. Spend at least {self.min_order_amount:,.0f} VND to activate."
            )
        ...
        if (
            ...
            and sales_channel
            and (sales_channel not in self.applicable_channels)
        ):
            return False, _(
                "This promotional code is invalid for your chosen electronic sales gateway interface."
            )
