...
class LoyaltyTransaction(BaseModel):
    ...
    TYPE_CHOICES = (
        ...
        (
            "EXPIRE",
            _("Expire - Unused points passing past chronological validity deadlines"),
        ),
        (
            "ADJUST",
            _(
    ...
    def clean(self):
        ...
        if self.type == "EARN" and self.points < 0:
            raise ValidationError(
                {
                    "points": _(
                        "Ledger Conflict: Accumulation operations ('EARN') cannot mathematically execute negative point values."
                    )
                }
            )
        ...
        if self.type == "REDEEM" and self.points > 0:
            raise ValidationError(
                {
                    "points": _(
                        "Ledger Conflict: Cash credit redemptions ('REDEEM') must reflect negative math deductions inside the points column."
                    )
                }
            )
        ...
        if self.type == "EXPIRE" and self.points > 0:
            raise ValidationError(
                {
                    "points": _(
                        "Ledger Conflict: Points expiration events ('EXPIRE') must reflect negative math deductions inside the points column."
                    )
                }
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if is_creating:
            ...
            with transaction.atomic():
                ...
                customer_profile = Customer.objects.select_for_update().get(
                    pk=self.customer_id
                )
                ...
                if computed_balance < 0:
                    raise ValidationError(
                        _(
                            "Wallet Starvation Exception: Operation denied. Customer points balance fails to clear zero bounds."
                        )
                    )
            ...
            raise ValidationError(
                _(
                    "Ledger Lock Exception: Historical points entries are immutable and cannot be updated."
                )
            )
