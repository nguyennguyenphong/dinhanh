...
class PaymentMethod(BaseModel):
    ...
    CODE_CHOICES = (
        ...
        (
            "BANK_TRANSFER",
            _("Bank Transfer - Automatic bank wire virtual accounts clearing"),
        ),
    ...
    def clean(self):
        ...
        if self.is_active and self.code not in ["CASH", "CREDIT"] and not self.config:
            raise ValidationError(
                {
                    "config": _(
                        "Integration Error: Activation of digital channels requires explicit API parameters mapping inside the config field."
                    )
                }
            )
