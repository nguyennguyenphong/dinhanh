...
class Payment(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        ('SUCCESS', _('Success - Settlement confirmed, capital moved into system vaults')),
        ('FAILED', _('Failed - Transaction aborted, rejected by network clearing lines')),
        ('REFUNDED', _('Refunded - Financial commercial reversal processed back to customer')),
        ('EXPIRED', _('Expired - Session gate timeout limit breached before completion')),
    )
    ...
    def clean(self):
        ...
        if not self.booking_id and not self.consignment_id:
            raise ValidationError(_("Financial Source Error: An inflow invoice line must be linked to at least one Ticket Booking or Cargo Consignment source."))
            
        if self.amount and self.amount <= 0:
            raise ValidationError({
                'amount': _('Financial Data Error: Inbound transaction amount value metrics must be positive figures.')
            })
    ...
    def execute_successful_settlement(self, external_ref, raw_callback_data=None):
        ...
        if self.status not in ['PENDING', 'EXPIRED']:
            raise ValidationError(_("State Machine Block: Cannot clear transactions that have already been terminated or processed."))
        ...
        self.save(update_fields=['status', 'transaction_ref', 'paid_at', 'gateway_response', 'updated_at'])
    ...
    def execute_failure_termination(self, failure_response_packet):
        ...
        if self.status in ['SUCCESS', 'REFUNDED']:
            raise ValidationError(_("Accounting Audit Error: Cannot drop an invoice line that has already completed settlement clearance."))
