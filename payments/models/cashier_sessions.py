...
class CashierSession(BaseModel):
    ...
    def clean(self):
        ...
        if self.opening_cash and self.opening_cash < 0:
            raise ValidationError(
                {
                    "opening_cash": _(
                        "POS Compliance Error: Opening float cash values cannot reflect negative figures."
                    )
                }
            )
    ...
    def execute_shift_close(self, counted_physical_cash):
        ...
        if self.status != "OPEN":
            raise ValidationError(
                _(
                    "POS Shift Block: Shift termination can only execute on an ACTIVE OPEN cashier session."
                )
            )
        ...
        aggregated_refunds = (
            0.00  # Replace with appropriate query linkage when necessary
        )

        # 3. Compute mathematically expected cash drawer balance metrics
        expected_cash_in_drawer = (
            self.opening_cash + aggregated_sales - aggregated_refunds
        )
    ...
    def execute_finance_reconciliation(self, auditor_user, manager_notes=""):
        ...
        if self.status != "CLOSED":
            raise ValidationError(
                _(
                    "Auditing Block: Sessions must bypass terminal employee shift CLOSE before review clearance."
                )
            )
        ...
        if manager_notes:
            self.notes = (
                f"{self.notes}\n[Audit Log - Managed by User #{auditor_user.id}]: {manager_notes}"
                if self.notes
                else f"[Audit Log]: {manager_notes}"
            )
