...
class Invoice(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        ('ISSUED', _('Issued - Digitally signed, authorized by tax authority portal, locked')),
        ('CANCELLED', _('Cancelled - Legally voided via official adjustment minute protocol')),
        ('REPLACED', _('Replaced - Voided and superseded by a corrective replacement invoice')),
    )
    ...
    def clean(self):
        ...
        if not self.booking_id and not self.group_contract_id:
            raise ValidationError(_("Tax Compliance Error: A tax document must be derived from an active retail Ticket Booking or a B2B Group Contract source."))
            
        # Precise rounding checks calculating VAT amounts: (Subtotal * VAT Rate) / 100
        if self.subtotal is not None and self.vat_rate is not None and self.vat_amount is not None:
            expected_vat = (self.subtotal * self.vat_rate) / 100
            if abs(self.vat_amount - expected_vat) > 1.00:  # Allow 1 VND tolerance factor due to tax decimal roundings
                raise ValidationError({
                    'vat_amount': _('Accounting Discrepancy: Calculated VAT amount parameters do not align with subtotal and applied tax weights.')
                })
                
        # Grand Total verification: Subtotal + VAT Amount = Total Amount
        if self.subtotal is not None and self.vat_amount is not None and self.total_amount is not None:
            expected_total = self.subtotal + self.vat_amount
            if abs(self.total_amount - expected_total) > 1.00:
                raise ValidationError({
                    'total_amount': _('Accounting Discrepancy: Grand total amount metric does not mathematically match the sum of subtotal and tax values.')
                })

    def execute_electronic_portal_issuance(self, accountant_user, gateway_code, dynamic_view_url):
        """
        ...
        if self.status != 'DRAFT':
            raise ValidationError(_("Tax Portal Error: Issuance pipelines can only trigger on raw DRAFT stage records."))
        ...
        self.save(update_fields=['status', 'e_invoice_code', 'e_invoice_url', 'issued_by', 'issued_at', 'updated_at'])
    ...
    def execute_legal_invoice_cancellation(self, adjustment_minute_ref):
        ...
        if self.status != 'ISSUED':
            raise ValidationError(_("Compliance Error: Voiding/Cancellation procedures can only operate on officially ISSUED tax documents."))
            
        self.status = 'CANCELLED'
        # Log the legal adjustments protocol token directly into tracking logs text fields
        self.buyer_address = f"{self.buyer_address}\n[VOIDED VIA TAX PROTOCOL MINUTE REF: {adjustment_minute_ref}]" if self.buyer_address else f"[VOIDED MINUTE: {adjustment_minute_ref}]"
