...
class Notification(models.Model):
    ...
    RECIPIENT_TYPE_CHOICES = (
        ('USER', _('User - Generic console accounts or internal admin portal entities')),
        ('CUSTOMER', _('Customer - Active consumer passengers or freight logistical clients')),
        ('EMPLOYEE', _('Employee - On-field fleet transport operational staff or station managers')),
    )

    STATUS_CHOICES = (
        ('PENDING', _('Pending - Staged in transmission queue, awaiting background daemon pickup')),
        ('SENT', _('Sent - External telecom/email gateway successfully authorized delivery')),
        ('FAILED', _('Failed - Gateway aborted transaction execution, stalled in error state')),
        ('CANCELLED', _('Cancelled - Voluntarily terminated by administrative task overrides before dispatch')),
    )
    ...
    def clean(self):
        ...
        if self.channel == 'EMAIL' and not self.recipient_email:
            raise ValidationError({
                'recipient_email': _('Routing Error: EMAIL channel dispatches require a filled destination recipient_email field.')
            })
            
        if self.channel in ['SMS', 'ZALO'] and not self.recipient_phone:
            raise ValidationError({
                'recipient_phone': _('Routing Error: Telecommunication channels (SMS/ZALO) require a filled recipient_phone field.')
            })
