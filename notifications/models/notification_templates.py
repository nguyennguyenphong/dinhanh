...
class NotificationTemplate(models.Model):
    ...
    CHANNEL_CHOICES = (
        ('SMS', _('SMS - Raw plaintext cellular carrier telecommunication short messages')),
        ...
        ('ZALO', _('Zalo - Official localized Zalo Notification Service (ZNS) message blocks')),
        ('IN_APP', _('In-App - Internal customer account dashboard activity notification bell rows')),
    )
    ...
    def clean(self):
        ...
        if self.channel == 'EMAIL' and not self.subject:
            raise ValidationError({
                'subject': _('Compliance Error: Rich text EMAIL blueprints require an explicit subject line definition header.')
            })
        ...
        if self.body and self.variables is not None:
            ...
            for placeholder in extracted_placeholders:
                if placeholder not in self.variables:
                    raise ValidationError({
                        'body': _(f"Compilation Discrepancy: Placeholder syntax '{{{placeholder}}}' detected inside body text but missing from authorized variable arrays.")
                    })
    ...
    def render_message_payload(self, context_data_dict):
        ...
        if not self.is_active:
            raise ValueError(_("Execution Block: Target template blueprint is currently disabled."))
        ...
        try:
            ...
            raise KeyError(_(f"Hydration Error: Required template rendering context key '{{{error.args[0]}}}' missing from data argument injection packet."))
