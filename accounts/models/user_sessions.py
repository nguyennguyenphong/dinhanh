...
class UserSession(models.Model):
    ...
    user_agent = models.TextField(help_text="User agent string (browser/device info)")
    ...
    last_activity = models.DateTimeField(
        auto_now=True, help_text="Last activity timestamp for this session"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["user", "revoked_at"], name="idx_session_user_active"),
            # Index for token lookups
            models.Index(fields=["session_token"], name="idx_session_token"),
            # Index for refresh token lookups
            models.Index(fields=["refresh_token"], name="idx_session_refresh_token"),
    ...
    def refresh_tokens(self):
        ...
        try:
            payload = jwt.decode(
                self.refresh_token, settings.SECRET_KEY, algorithms=["HS256"]
            )
    ...
    def verify_token(token):
        ...
        try:
            ...
            session = UserSession.objects.get(
                id=payload["session_id"], session_token=token
            )
    ...
    def verify_refresh_token(token):
        ...
        try:
            ...
            # Get session
            session = UserSession.objects.get(
                id=payload["session_id"], refresh_token=token
            )
    ...
    def cleanup_expired_sessions(cls):
        ...
        """
        now = timezone.now()
        expired_sessions = cls.objects.filter(expires_at__lt=now)
