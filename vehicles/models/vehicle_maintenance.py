...
class VehicleMaintenance(models.Model):
        ...
        if self.odometer_in and self.odometer_out:
            if self.odometer_out < self.odometer_in:
                raise ValidationError({
                    'odometer_out': _('Check-out odometer cannot be lower than check-in mileage.')
                })
    ...
    def complete_maintenance(self, final_odometer, final_cost=None):
        ...
        self.save(update_fields=['odometer_out', 'cost', 'status', 'completed_at', 'updated_at'])
    ...
    def get_total_expenses(cls, tenant_id, start_date=None, end_date=None):
        ...
        return cls.objects.filter(query).aggregate(total=models.Sum('cost'))['total'] or 0.00