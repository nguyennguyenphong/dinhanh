...
class ReportDefinition(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=models.Q(
                    category__in=["REVENUE", "OPERATIONS", "HR", "CARGO", "CUSTOMERS"]
                ),
        ...
        if self.is_builtin and self.id:
            ...
            if not original.is_builtin:
                raise ValidationError(
                    {
                        "is_builtin": _(
                            "Security Enforcement Error: Injecting custom runtime configurations into system infrastructure blocks is disallowed."
                        )
                    }
                )
    ...
    def execute_report_data_compile(self, execution_engine_callback):
        ...
        if not self.query_config:
            raise ValueError(
                _(
                    "Execution Exception: Cannot parse report query due to empty query_config parameters."
                )
            )
