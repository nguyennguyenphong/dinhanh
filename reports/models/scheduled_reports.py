...
class ScheduledReport(models.Model):
    ...
    FREQUENCY_CHOICES = (
        ('DAILY', _('Daily - Dispatches compiled blueprints once every calendar day loop')),
        ('WEEKLY', _('Weekly - Executes summary analytical matrices once every seven calendar days')),
        ('MONTHLY', _('Monthly - Fires off deep period balance sheet summaries at month-end close dates')),
    )

    FORMAT_CHOICES = (
        ('PDF', _('PDF - Portable Document Format layout configured for print-ready layout reviews')),
        ('EXCEL', _('Excel - Structured digital spreadsheet workbook optimized for manual data audits')),
        ('CSV', _('CSV - Raw comma-separated values document tailored for external analytical processing pipelines')),
    )
        ...
        if self.cron_expr:
            ...
            if len(cron_parts) != 5:
                raise ValidationError({
                    'cron_expr': _('Syntax Parsing Exception: Cron expressions must contain exactly 5 space-separated operational field values (Minute, Hour, Day-of-Month, Month, Day-of-Week).')
                })
        ...
        if isinstance(self.recipients, list):
            for idx, entry in enumerate(self.recipients):
                if not isinstance(entry, dict) or 'type' not in entry or 'value' not in entry:
                    raise ValidationError({
                        'recipients': _(f"Envelope Shape Error at item index [{idx}]: Elements inside delivery lists must match standard dict keys: 'type' and 'value'.")
                    })
                if entry['type'] not in ['email', 'user']:
                    raise ValidationError({
                        'recipients': _(f"Validation Exception at item index [{idx}]: Delivery routing types must restrict to 'email' or 'user' taxonomy rules.")
                    })
        else:
            raise ValidationError({
                'recipients': _('Structural Typing Error: Target recipient fields must be saved under a valid flat array collection schema.')
            })
