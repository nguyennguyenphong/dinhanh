...
class MenuItemRole(models.Model):
    ...
    def clean(self):
        ...
        if self.menu_item.tenant_id != self.role.tenant_id:
            raise ValidationError(
                'Menu item and role must belong to same tenant'
            )
    ...
    def is_visible_to_role(cls, menu_item, role):
        ...
        return menu_item.role_assignments.filter(
            role=role
        ).exists()
...
class MenuItemRoleAuditLog(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=['action'],
                name='idx_menu_item_role_audit_action'
            ),
        ]
    ...
    @classmethod
    def log_action(cls, tenant, menu_item, role=None, action='ASSIGN',
                  actor=None, actor_username=None, affected_count=1,
                  reason=None):
        """
