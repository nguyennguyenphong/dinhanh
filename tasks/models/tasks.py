...
class Task(models.Model):
    ...
    description = models.TextField(
        blank=True, null=True, help_text="Detailed task description"
    )
    ...
    due_date = models.DateTimeField(
        null=True, blank=True, db_index=True, help_text="Due date for task"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, db_index=True, help_text="When task was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When task was last updated"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["assignee_id"], name="idx_tasks_assignee"),
            # Index for entity linking
            models.Index(fields=["entity_type", "entity_id"], name="idx_tasks_entity"),
            ...
            models.Index(fields=["due_date", "status"], name="idx_tasks_due_status"),
    ...
    def get_active_tasks(cls, tenant=None):
        ...
        """
        query = cls.objects.filter(status__in=["TODO", "IN_PROGRESS", "REVIEW"])
    ...
    def get_entity_tasks(cls, entity_type, entity_id, status=None):
        ...
        """
        query = cls.objects.filter(entity_type=entity_type, entity_id=entity_id)
    ...
    def bulk_update_status(cls, task_ids, new_status):
        ...
        if new_status == "DONE":
            return cls.objects.filter(id__in=task_ids).update(
                status=new_status, completed_at=timezone.now()
            )

        return cls.objects.filter(id__in=task_ids).update(status=new_status)
    ...
    def bulk_assign(cls, task_ids, assignee):
        ...
        """
        return cls.objects.filter(id__in=task_ids).update(assignee=assignee)
    ...
    def bulk_set_priority(cls, task_ids, priority):
        ...
        """
        return cls.objects.filter(id__in=task_ids).update(priority=priority)
    ...
    def bulk_set_due_date(cls, task_ids, due_date):
        ...
        """
        return cls.objects.filter(id__in=task_ids).update(due_date=due_date)
    ...
    def get_statistics(cls, tenant=None):
        ...
        completion_rate = (
            (done / (total - cancelled) * 100) if (total - cancelled) > 0 else 0
        )
