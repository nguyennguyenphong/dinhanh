...
class RouteStop(models.Model):
    ...
    def get_ordered_stops_by_route(cls, route_id):
        ...
        """
        return cls.objects.filter(route_id=route_id).select_related('station').order_by('stop_order')
    ...
    def get_routes_by_station_pair(cls, origin_station_id, destination_station_id):
        ...
        return Route.objects.filter(
            id__in=cls.objects.filter(station_id=origin_station_id).values('route_id')
        ).filter(
