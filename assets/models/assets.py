...
class Asset(models.Model):
    ...
    STATUS_CHOICES = (
        ('IN_USE', _('In Use - Asset is operational and deployed inside company workflows')),
        ('MAINTENANCE', _('Maintenance - Asset is offline undergoing technical service or workshop repair')),
        ('DISPOSED', _('Disposed - Asset is retired from accounting books via scrap sale or liquidation')),
        ('LOST', _('Lost - Asset is missing from physical audits; flagged for financial investigation')),
        ('TRANSFERRED', _('Transferred - Asset is locked mid-transit during relocation between operational branches')),
    )
    ...
    def clean(self):
        ...
        if self.category_id and self.tenant_id != self.category.tenant_id:
            raise ValidationError({
                'category': _('Cross-Tenant Data Leak Error: Target asset category catalog belongs to a different tenant.')
            })
        ...
        if self.purchase_price and self.current_value:
            if self.current_value > self.purchase_price:
                raise ValidationError({
                    'current_value': _('Fiscal Valuation Anomaly: Current net book value cannot exceed the historical capitalized procurement cost.')
                })
        ...
        if self.purchase_date and self.warranty_expiry:
            if self.warranty_expiry < self.purchase_date:
                raise ValidationError({
                    'warranty_expiry': _('Calendar Chronology Error: Manufacturer warranty expiration dates must occur after the official procurement date.')
                })
    ...
    def calculate_straight_line_depreciation(self, target_year_end_date):
        ...
        """
        if not self.purchase_date or not self.purchase_price or not self.depreciation_rate:
        ...
        total_depreciation = self.purchase_price * (self.depreciation_rate / Decimal('100.00')) * years_held
