...
class Customer(SafeDeleteModel):
    ...
    SOURCE_CHOICES = (
        ...
        (
            "IMPORT",
            _("Import - Administrative spreadsheet or legacy DB data migration"),
        ),
    )
    ...
    def clean(self):
        ...
        if self.loyalty_points and self.loyalty_points < 0:
            raise ValidationError(
                {
                    "loyalty_points": _(
                        "Loyalty metrics error: The wallet account point scale cannot reflect negative parameters."
                    )
                }
            )
    ...
    def deduce_loyalty_rewards(self, spent_points, audit_log_reason):
        ...
        if self.loyalty_points < spent_points:
            raise ValidationError(
                _(
                    "Insufficient loyalty balance coordinates available to perform the requested extraction code."
                )
            )
        ...
        append_note = (
            f"[Points Redeemed]: -{spent_points} pts | Reason: {audit_log_reason}"
        )
        self.notes = f"{self.notes}\n{append_note}" if self.notes else append_note
