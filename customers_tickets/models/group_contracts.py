...
class GroupContract(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        ('CONFIRMED', _('Confirmed - Deposit cleared, passenger block inventory secured')),
        ('CANCELLED', _('Cancelled - Contract terminated, seat allocation blocks released')),
        ('COMPLETED', _('Completed - Journey finalized, invoice balances cleared and locked')),
    )
        ...
        if self.total_amount is not None and self.deposit_amount is not None:
            if self.deposit_amount > self.total_amount:
                raise ValidationError({
                    'deposit_amount': _('Accounting Logic Error: Milestone deposit amount cannot physically exceed the contract gross total amount.')
                })
                
        if self.seat_count and self.seat_count <= 0:
            raise ValidationError({
                'seat_count': _('Logistics Error: Block allocation requests must allocate 1 or more seats.')
            })
    ...
    def execute_deposit_clearance(self):
        ...
        if self.status != 'DRAFT':
            raise ValidationError(_("Workflow Block: Deposit clearance milestone can only be executed on DRAFT stage contracts."))
    ...
    def finalize_contract_completion(self):
        ...
        if self.status != 'CONFIRMED':
            raise ValidationError(_("Operational Error: Contracts cannot transition to COMPLETED without going through CONFIRMED status."))
            
        if not self.contract_file:
            raise ValidationError({
                'contract_file': _('Compliance Audit Alert: Cannot close out a contract line without attaching the signed contract PDF scan copy file.')
            })
    ...
    def execute_contract_cancellation(self, void_reason):
        ...
        if self.status in ['CANCELLED', 'COMPLETED']:
            raise ValidationError(_("State Machine Error: This group contract record is already closed or terminated."))
