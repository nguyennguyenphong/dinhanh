...
class Ticket(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        ('REFUNDED', _('Refunded - Terminated with processed banking financial reversal')),
        ('EXCHANGED', _('Exchanged - Voided due to seat/trip reassignment modification')),
    ...
    def clean(self):
        ...
        if self.base_price is not None and self.discount_amount is not None and self.final_price is not None:
            calculated_price = self.base_price - self.discount_amount
            if calculated_price < 0:
                calculated_price = 0
            if abs(self.final_price - calculated_price) > 0.01:
                raise ValidationError({
                    'final_price': _('Financial Arithmetic Discrepancy: Final price must match (base_price - discount_amount) calculation flow.')
                })
    ...
    def execute_terminal_gate_checkin(self, gate_staff_user):
        ...
        if self.status != 'ACTIVE':
            raise ValidationError(_("Gate Clearance Error: Boarding pass is invalid. Current ticket lifecycle token is: {}").format(self.status))
