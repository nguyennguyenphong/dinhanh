...
class TicketRefund(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        (
            "APPROVED",
            _("Approved - Clearance authorized, payout transactional task queued"),
        ),
        ("COMPLETED", _("Completed - Capital successfully transferred, ledger locked")),
        (
            "REJECTED",
            _("Rejected - Request declined, booking ticket restored to active status"),
        ),
    )
    ...
    def clean(self):
        ...
        if (
            ...
            and self.refund_amount is not None
        ):
            ...
            if abs(self.refund_amount - expected_payout) > 0.01:
                raise ValidationError(
                    {
                        "refund_amount": _(
                            "Accounting Arithmetic Error: Refund payout must equal (original_amount - penalty_amount) calculation matrix."
                        )
                    }
                )
    ...
    def execute_supervisor_approval(self, supervisor_user):
        ...
        if self.status != "PENDING":
            raise ValidationError(
                _(
                    "Review Block: This refund line document has already bypassed its reviewable PENDING state."
                )
            )
        ...
        if self.processed_by == supervisor_user:
            raise ValidationError(
                _(
                    "Compliance Fraud Block: The supervisor initiating the refund (Maker) cannot act as the approver (Checker)."
                )
            )
    ...
    def execute_final_payout_completion(self):
        ...
        if self.status != "APPROVED":
            raise ValidationError(
                _(
                    "Payout Processing Error: Payout cannot execute without prior supervisor APPROVED clearance marks."
                )
            )
