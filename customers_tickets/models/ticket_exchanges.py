...
class TicketExchange(models.Model):
    ...
    STATUS_CHOICES = (
        ('PENDING', _('Pending - Rerouting initialized, awaiting fee payment settlement')),
        ('COMPLETED', _('Completed - Fee cleared, original ticket revoked, new ticket activated')),
        ('REJECTED', _('Rejected - Exchange request cancelled, original ticket state retained')),
    )
    ...
    def clean(self):
        ...
        if self.original_ticket_id and self.new_ticket_id and self.original_ticket_id == self.new_ticket_id:
            raise ValidationError({
                'new_ticket': _("Data Logic Error: Target replacement ticket cannot be physically identical to the source original ticket.")
            })
            
        if self.fee and self.fee < 0:
            raise ValidationError({
                'fee': _("Financial validation error: Modification processing surcharges cannot reflect negative parameters.")
            })
    ...
    def execute_exchange_confirmation(self, administrative_staff_user=None):
        ...
        if self.status != 'PENDING':
            raise ValidationError(_("Workflow Block: This exchange entry ledger line has already bypassed its reviewable PENDING state."))
            
        if not self.new_ticket:
            raise ValidationError(_("Operational Error: Cannot complete migration flow without an explicitly attached new replacement ticket."))
    ...
    def execute_exchange_rejection(self, notes_reason=""):
        ...
        """
        if self.status != 'PENDING':
            raise ValidationError(_("State Machine Error: This mutation pipeline record is already closed or locked."))
            
        self.status = 'REJECTED'
        if notes_reason:
            self.reason = f"{self.reason}\n[Rejection Reason]: {notes_reason}" if self.reason else f"[Rejection Reason]: {notes_reason}"
