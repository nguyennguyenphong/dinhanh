...
class Payroll(models.Model):
    ...
    def clean(self):
        ...
        if self.period_month < 1 or self.period_month > 12:
            raise ValidationError({'period_month': _('The period month must fall strictly between indices 1 and 12.')})
            
        if self.status == 'PAID' and not self.paid_at:
            raise ValidationError({'paid_at': _('A statement flagged as PAID must feature an explicit transaction execution timestamp.')})
    ...
    def lock_and_approve(self, supervisor_user):
        ...
        if self.status != 'DRAFT':
            raise ValidationError(_("Only statements in DRAFT can be locked and transitioned to APPROVED."))
    ...
    def mark_as_disbursed(self):
        ...
        if self.status != 'APPROVED':
            raise ValidationError(_("Payroll statements must be explicitly APPROVED by management before registering payouts."))
    ...
    def get_total_monthly_payout_volume(cls, tenant_id, year, month):
        ...
        ).aggregate(total_net=models.Sum('net_salary'))['total_net'] or 0.00