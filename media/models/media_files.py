...
class MediaFile(models.Model):
    ...
    @classmethod
    def create_from_upload(cls, tenant, file, uploaded_by=None,
                          entity_type=None, entity_id=None,
                          is_public=False, storage='local'):
        """
    ...
    def _save_to_s3(self, file):
        ...
        s3.upload_fileobj(
            file,
            self.bucket or settings.AWS_STORAGE_BUCKET_NAME,
            self.file_path
        )
    ...
    def _delete_from_s3(self):
        ...
        s3 = boto3.client('s3')
        s3.delete_object(
            Bucket=self.bucket or settings.AWS_STORAGE_BUCKET_NAME,
            Key=self.file_path
        )
