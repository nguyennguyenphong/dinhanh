    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=['tenant', 'code'],
                name='uq_tenant_expense_category_code'
            )
        ]
        ...
        if self.id and self.parent_id:
            ...
            if self.id == self.parent_id:
                raise ValidationError({
                    'parent': _('Hierarchical Integrity Error: A financial expense category cannot mathematically assign itself as its own parent group.')
                })
            ...
            if self.tenant_id != self.parent.tenant_id:
                raise ValidationError({
                    'parent': _('Cross-Tenant Data Leak Error: Target parent classification group resides inside a different tenant data silo boundary.')
                })
            ...
            while current_parent is not None:
                ...
                if nesting_depth > 3:
                    raise ValidationError({
                        'parent': _('Operational Limit Error: System restrictions enforce a maximum architectural depth of 3 nested catalog layers.')
                    })
