...
class TripPrice(models.Model):
    ...
    def clean(self):
        ...
        if self.price and self.price < 0:
            raise ValidationError({'price': _('The base standard adult price cannot be a negative scalar value.')})
            
        if self.child_price and self.child_price < 0:
            raise ValidationError({'child_price': _('The concessionary child price cannot be a negative scalar value.')})
            
        if self.price and self.child_price and self.child_price > self.price:
            raise ValidationError({
                'child_price': _('Business Compliance Alert: Child concessionary price cannot exceed adult base pricing.')
            })
        ...
        if self.valid_from and self.valid_to:
            if self.valid_from > self.valid_to:
                raise ValidationError({
                    'valid_to': _('The valid expiration date boundary cannot precede its effective activation date.')
                })
    ...
    def get_active_tariff(cls, tenant_id, route_id, seat_type, target_date):
        ...
        ).order_by('-valid_from').first()  # Grabs the most specific/recently activated tariff if overlapping rules exist