...
from django.contrib.postgres.fields import ArrayField  # Production feature for PostgreSQL SMALLINT[]
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=['tenant', 'code'],
                name='unique_tenant_trip_schedule_code'
            ),
        ]
    ...
    def clean(self):
        ...
        for day in self.days_of_week:
            if day < 1 or day > 7:
                raise ValidationError({
                    'days_of_week': _('Day index trackers must reside strictly within range limits 1 (Mon) and 7 (Sun).')
                })
        ...
        if self.valid_from and self.valid_to:
            if self.valid_from > self.valid_to:
                raise ValidationError({
                    'valid_to': _('The valid closure timeline date cannot precede its opening effective date.')
                })
