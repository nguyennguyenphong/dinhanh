...
class DispatchOrder(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        ('ISSUED', _('Issued - Dispatch cleared, vehicle ready for terminal departure')),
    ...
    def clean(self):
        ...
        if self.status in ['ISSUED', 'DEPARTED'] and not self.issued_by:
            raise ValidationError({
                'issued_by': _('Operational Compliance Alert: An order cannot transition to ISSUED/DEPARTED without an authorizing supervisor signature.')
            })
    ...
    def execute_clearance_release(self, supervisor_user):
        ...
        if self.status != 'PENDING':
            raise ValidationError(_("This dispatch order document has already bypassed its reviewable PENDING state."))
    ...
    def register_gate_departure(self):
        ...
        if self.status != 'ISSUED':
            raise ValidationError(_("Gate House Error: Vehicle cannot pass exit gate without a verified ISSUED clearance certificate."))
