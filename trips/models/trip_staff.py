...
class TripStaff(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                ...
                name='unique_trip_employee_assignment'
            ),
        ]
    ...
    def clean(self):
        ...
        if self.role == 'DRIVER' and not self.employee.is_driver():
            raise ValidationError({
                'employee': _('The selected personnel cannot act as a DRIVER due to missing position qualifications.')
            })
        ...
        if self.role == 'DRIVER' and self.employee.has_expired_license():
            raise ValidationError({
                'employee': _('Safety Compliance Block: This driver holds an expired commercial vehicle driving license.')
            })
    ...
    def get_trip_crew_manifest(cls, trip_id):
        ...
        """
        return cls.objects.filter(trip_id=trip_id).select_related('employee', 'shift_type').order_by('role')