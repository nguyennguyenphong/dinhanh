...
from tenants.serializers.tenants.tenant_list_query_serializer import TenantListQuerySerializer
from tenants.serializers.tenants.tenant_response_serializer import TenantResponseSerializer
...
class TenantListView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        grid_builder = DjangoGridBuilder(
            grid_id="tenant-grid",
            api_url=reverse("tenant_list_api"),
            page_size=50
        )
        ...
        grid_builder.add_column("logo_url", "Logo", col_type="image", width=100, sortable=False, filter=False)
        ...
        grid_builder.add_column("max_branches", "Max Branches", col_type="number", width=130)
        grid_builder.add_column("max_vehicles", "Max Vehicles", col_type="number", width=130)
        grid_builder.add_column("primary_color", "Màu chính", col_type="text", width=130)
        grid_builder.add_column("default_language", "Ngôn ngữ", col_type="text", width=130)
        ...
        grid_builder.add_column("updated_at", "Ngày cập nhật", col_type="datetime", width=180)
        grid_builder.add_column("deleted_at", "Ngày xóa", col_type="datetime", width=180)
