...
class Tag(models.Model):
    ...
    def search_tags(cls, tenant, query):
        ...
        return cls.objects.filter(tenant=tenant).filter(
            Q(label__icontains=query) | Q(slug__icontains=query)
        ).order_by('label')
...
class TaggedItem(models.Model):
    ...
    def tag_entity(cls, tag, entity_type, entity_id):
        tagged, created = cls.objects.get_or_create(tag=tag, entity_type=entity_type, entity_id=entity_id)
    ...
    def get_entity_tags(cls, entity_type, entity_id):
        return Tag.objects.filter(tagged_items__entity_type=entity_type, tagged_items__entity_id=entity_id).distinct()

    @classmethod
    def get_tagged_items(cls, tag, entity_type=None):
        query = cls.objects.filter(tag=tag)
        if entity_type: query = query.filter(entity_type=entity_type)
    ...
    def get_common_tags(cls, entity_type, entity_ids):
        ...
        return Tag.objects.filter(tagged_items__entity_type=entity_type, tagged_items__entity_id__in=entity_ids).annotate(
            count=Count('id')
        ).filter(count=len(entity_ids)).distinct()

    @classmethod
    def get_tags_with_counts(cls, tenant, entity_type=None):
        from django.db.models import Count
        query = Tag.objects.filter(tenant=tenant).annotate(usage_count=Count('tagged_items')).order_by('-usage_count')
        if entity_type: query = query.filter(tagged_items__entity_type=entity_type).distinct()
