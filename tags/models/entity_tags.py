...
class EntityTag(models.Model):
    ...
    def has_tag(cls, entity_type, entity_id, tag):
        if isinstance(tag, str):
            return cls.objects.filter(entity_type=entity_type, entity_id=entity_id, tag__slug=tag).exists()
        return cls.objects.filter(entity_type=entity_type, entity_id=entity_id, tag=tag).exists()
    ...
    def has_any_tag(cls, entity_type, entity_id, tags):
        ...
        return cls.objects.filter(entity_type=entity_type, entity_id=entity_id).filter(
            Q(tag_id__in=tag_ids) | Q(tag__slug__in=tag_slugs)
        ).exists()

    @classmethod
    def has_all_tags(cls, entity_type, entity_id, tags):
        return cls.objects.filter(entity_type=entity_type, entity_id=entity_id).count() == len(tags)
    ...
    def get_tagged_entity_ids(cls, tag, entity_type):
        return list(cls.objects.filter(tag=tag, entity_type=entity_type).values_list('entity_id', flat=True))
    ...
    def get_tagged_entities_with_info(cls, tag, entity_type):
        entity_tags = cls.objects.filter(tag=tag, entity_type=entity_type).select_related('tagged_by').order_by('-tagged_at')
        return [{
    ...
    def get_common_tags(cls, entity_type, entity_ids):
        ...
        ).annotate(count=Count('id')).filter(count=len(entity_ids)).distinct()

    @classmethod
    def get_tags_with_counts(cls, entity_type):
        from tags.models.tags import Tag
        tags = Tag.objects.filter(entity_tags__entity_type=entity_type).annotate(
    ...
    def tag_entity(cls, tag, entity_type, entity_id, tagged_by=None):
        entity_tag, created = cls.objects.get_or_create(
            ...
            defaults={'tagged_by': tagged_by, 'tagged_by_username': tagged_by.username if tagged_by else None}
        )
        return entity_tag, created

    @classmethod
    def untag_entity(cls, tag, entity_type, entity_id):
        count, _ = cls.objects.filter(tag=tag, entity_type=entity_type, entity_id=entity_id).delete()
        return count

    @classmethod
    def bulk_tag_entity(cls, tags, entity_type, entity_id, tagged_by=None):
        return [cls.tag_entity(tag, entity_type, entity_id, tagged_by)[0] for tag in tags]
    ...
    def clear_entity_tags(cls, entity_type, entity_id):
        count, _ = cls.objects.filter(entity_type=entity_type, entity_id=entity_id).delete()
        return count
    ...
    def bulk_tag_entities(cls, tag, entity_type, entity_ids, tagged_by=None):
        return [cls.tag_entity(tag, entity_type, eid, tagged_by)[0] for eid in entity_ids]

    @classmethod
    def bulk_untag_entities(cls, tag, entity_type, entity_ids):
        count, _ = cls.objects.filter(tag=tag, entity_type=entity_type, entity_id__in=entity_ids).delete()
        return count
    ...
    def get_entity_tag_stats(cls, entity_type, entity_id):
        entity_tags = cls.objects.filter(entity_type=entity_type, entity_id=entity_id).select_related('tag', 'tagged_by').order_by('-tagged_at')
    ...
    def get_user_tagging_activity(cls, user, days=30):
        ...
        return {'total_tags': entity_tags.count(), 'by_entity_type': by_type, 'period_days': days}