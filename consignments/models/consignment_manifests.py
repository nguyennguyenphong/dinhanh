...
class ConsignmentManifest(models.Model):
    ...
    STATUS_CHOICES = (
        ('OPEN', _('Open - Active batching workspace, accepting package additions and scans')),
        ('CLOSED', _('Closed - Sealed batch sheet, cargo locked, awaiting yard driver gate release')),
        ('DISPATCHED', _('Dispatched - Vehicle cleared terminal gates, manifest is actively in transit')),
    )
    ...
    def execute_manifest_closure(self):
        ...
        """
        if self.status != 'OPEN':
            raise ValidationError(_("Logistics Exception: Seal/Closure operations can only trigger on active OPEN workspace manifests."))
        ...
        if not self.consignments.exists():
            raise ValidationError(_("Operations Error: Cannot close an empty manifest sheet. Scan or assign parcels to vehicle load grids first."))
    ...
    def execute_gate_dispatch(self):
        ...
        if self.status != 'CLOSED':
            raise ValidationError(_("Logistics Exception: Gate release protocols require the target manifest document to be fully CLOSED and sealed."))
