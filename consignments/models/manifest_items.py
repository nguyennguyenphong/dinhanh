...
class ManifestItem(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=['manifest', 'consignment'],
                name='uq_manifest_consignment_item'
            )
        ]
    ...
    def clean(self):
        ...
        if self.manifest_id and self.manifest.status != 'OPEN':
            raise ValidationError(_("Logistics Security Error: Cannot add or load parcels into a manifest that is already CLOSED or DISPATCHED."))
        ...
        if self.manifest_id and self.consignment_id:
            ...
            if self.consignment.trip_id and self.consignment.trip_id != self.manifest.trip_id:
                raise ValidationError(_("Routing Conflict: Target parcel is already routing scheduled for a different fleet journey transaction."))
