...
class CodReconciliation(BaseModel):
    ...
    def clean(self):
        ...
        if self.consignment_id and self.consignment.cod_amount <= 0:
            raise ValidationError(
                {
                    "consignment": _(
                        "Compliance Error: The linked cargo parcel does not possess a registered cash-on-delivery collection parameter."
                    )
                }
            )
        ...
        if (
            self.consignment_id
            and self.amount
            and self.amount != self.consignment.cod_amount
        ):
            raise ValidationError(
                {
                    "amount": _(
                        "Accounting Discrepancy: Payout allocation cannot deviate from the parent consignment's collected cod_amount."
                    )
                }
            )

    def execute_bank_transfer(
        self, accountant_user, transaction_reference_hash, custom_notes=""
    ):
        """
        ...
        if self.status != "PENDING":
            raise ValidationError(
                _(
                    "Financial Exception: Bank transfer execution requires a PENDING reconciliation leaf entry."
                )
            )
    ...
    def execute_final_clearance(self):
        ...
        if self.status != "TRANSFERRED":
            raise ValidationError(
                _(
                    "Financial Exception: Clearance confirmation requests require the record to sit under a TRANSFERRED state phase."
                )
            )
