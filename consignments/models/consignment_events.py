...
class ConsignmentEvent(models.Model):
    ...
    EVENT_TYPE_CHOICES = (
        ('STATUS_CHANGE', _('Status Change - State mutation tracking timeline adjustments')),
        ('NOTE', _('Note - Hand-written operational text memo or customer alert annotations')),
        ('SCAN', _('Scan - Hardware scanner laser gun ping logging warehouse floor checkpoint presence')),
        ('PHOTO', _('Photo - Optical imagery attachment asset tracking packaging structural integrity')),
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=['consignment', '-recorded_at'],
                name='idx_consignment_events'
            )
        ]
    ...
    def clean(self):
        ...
        if self.event_type == 'STATUS_CHANGE' and not self.new_status:
            raise ValidationError({
                'new_status': _('Compliance Discrepancy: Type is marked STATUS_CHANGE but no target new_status code parameter is provided.')
            })